...
import { SwahiliReportPDF } from "@/components/reports/SwahiliReportPDF"
...
interface SwahiliReportModalProps {
  scan: ScanSession | null;
  reports: SwahiliReport[] | null;
  onClose: () => void;
}

export function SwahiliReportModal({ scan, reports, onClose }: SwahiliReportModalProps) {
    ...
    <Dialog open={!!scan && !!reports} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl h-[90vh] flex flex-col overflow-hidden">
        <DialogHeader className="pb-4 border-b">
          <DialogTitle className="text-xl font-semibold">Swahili Security Report</DialogTitle>
        </DialogHeader>
        ...
        <DialogFooter className="mt-4 pt-4 border-t flex justify-between">
          {scan && reports && (
            <PDFDownloadLink
              ...
              fileName={`swahili-report-${scan.url.replace(/[^a-z0-9]/gi, '-')}.pdf`}
            >
              {({ loading }) => (
